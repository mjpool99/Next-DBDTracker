...
export default function TopKillers(props) {
  const data = props?.data || {killer: "name"};
  let newArray1 = [];
  let mostPlayed;

  Object.keys(data).map((key, index) => {
    return newArray1.push(data[index]?.killer)
  })
  const count = {};
  let mostCommon = [];
          ...
          <tbody>
            <tr>
              <td className="single-record-name text-center">
                {mostPlayed}: {data.filter(killer => killer.killer == mostPlayed).length}
              </td>
            </tr>
          </tbody>
        )}
