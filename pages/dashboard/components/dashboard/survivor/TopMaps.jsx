...
export default function TopMaps(props) {
  const data = props?.data || {map: "name"};
  let newArray1 = [];
  let mostPlayed;

  Object.keys(data).map((key, index) => {
    return newArray1.push(data[index]?.map)
  })
  const count = {};
  let mostCommon = [];
          ...
          <tbody>
            <tr>
              <td className="single-record-name text-center">
                {mostPlayed}: {data.filter(map => map?.map == mostPlayed).length}
              </td>
            </tr>
          </tbody>
        )}
